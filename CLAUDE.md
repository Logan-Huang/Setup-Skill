...
- **Directions conventions — one shared file plus a per-phase file.** Cross-skill conventions live in **`skills/astrodb-directions.md`** (the artifact-folder convention, the decision-log entry format + what-to-log, the general verify-and-report completion-checklist behavior, and "ask, don't assume"), symlinked into every skill as `references/astrodb-directions.md`. Two things that vary by phase live in a **per-phase** file — **`astrodb-build-directions.md`**, **`astrodb-ingest-directions.md`**, or **`astrodb-website-directions.md`** (symlinked as `references/astrodb-<phase>-directions.md`): the exact **decision-log file** (each phase keeps its own in its artifact directory — `astrodb-build-artifacts/build-workflow.md` and `astrodb-website-artifacts/website-workflow.md` appended; `astrodb-ingest-artifacts/ingest-workflow.md` prepended and dated; no shared project-root file) and **whether the phase persists its checklist**. Each skill's "Step 0: Read context documents" reads both its shared and its phase file.
- **Completion checklist** (developer guidance — the audience here is whoever edits these skills, not the end user): every `SKILL.md` ends with a `## Completion Checklist`, a blocking self-verification gate the skill must satisfy (or have the user explicitly waive) before reporting itself done. For **every** skill it is verified and reproduced (evidence-annotated) in the completion message. The **build** phase additionally **persists** it to a single `astrodb-build-artifacts/checklists.md` (one `## <skill-name>` section per skill, ticked with evidence) because that phase spans six skills; the **ingest** and **website** phases **do not persist** it — the ingest helpers fail loudly when a precondition isn't met, and website is a single skill, so a persisted file would be bloat. Items are verifiable outcomes; user-dependent ones read "prompted and honored the user's choice (or they declined)," never a forced action that could tempt fabrication. The general protocol lives in `astrodb-directions.md`; the persistence detail lives in each phase's `astrodb-<phase>-directions.md`; each `SKILL.md` keeps only its own items plus a short pointer. **Whenever you add, remove, or reorder a skill's steps, update that skill's checklist items in the same edit** — the checklist is the skill's contract, and a step with no checklist item is one that will silently get skipped.
