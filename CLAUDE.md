...
- **Problem log (`gotchas.md`)**: a root-level file, sibling to `workflow.md`, where every skill records problems caused by the *skills themselves* — instructions that were wrong or missing a case, a dependency behaving differently than documented, a step that reported success while producing a wrong result. Dataset-specific reasoning stays in `workflow.md`; `gotchas.md` is addressed to this repo's maintainers. Each entry is self-classified **`gotcha`** (would recur for another user or dataset — worth reporting) or **`one-off`** (specific to this run), carries a concrete `Suggested change`, and is formatted so it can be pasted into a GitHub issue as-is. When a run produces gotchas, the skill names them in its completion message and points the user at <https://github.com/astrodbtoolkit/astrodb-bot/issues/new>; skills never open an issue or PR themselves and never invoke `gh`. The protocol lives once in `astrodb-directions.md`; each `SKILL.md` carries one checklist item pointing at it.
